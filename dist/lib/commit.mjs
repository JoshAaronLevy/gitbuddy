...
const gitPushStep = (message) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (p_1.exitCode === 128) {
            spinner.warn({
                text: yellow(bold("ALERT! ")) +
                    white(`${currentBranch} branch does not exist in remote repository. Attempting to create and push upstream...`)
            });
            ...
            spinner.error({
                text: red(bold("ERROR! ")) +
                    white("Could not push to remote repository. See details below:\n" +
                        `${p_1.all}`)
            });
        }
    }
