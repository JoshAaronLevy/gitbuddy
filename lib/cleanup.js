...
function branchFeeder(deleteBranchList) {
...
const deleteBranchesCommand = async function (currentDeleteBranch) {
	...
	try {
		...
		return branchFeeder(deleteBranchList);
	} catch (error) {
...
const identifyLocalBranches = async function () {
	...
	try {
		...
		for (let i = 0; i < branchList.length; i++) {
			...
			if (branchList[i].startsWith('* ') === false && branchList[i] != 'master' && branchList[i] != 'main') {
...
/* ---------------------
GitBuddy Cleanup prompts
--------------------- */
const listBranches = new MultiSelect({
	type: 'checkbox',
	name: 'branchList',
	message: '1/2: Select the local branches you would like to delete (Space to select)',
	choices: availableBranches
});

const confirmBranchDelete = new Select({
	name: 'confirmBranchDelete',
	message: '2/2: Are you sure you wish to delete the selected branch(es)? This cannot be undone',
	choices: ['Yes', 'No']
});
