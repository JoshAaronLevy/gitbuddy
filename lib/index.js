...
const stagingCommand = async function(answer) {
  ...
  try {
    ...
    spinner.succeed(
      chalk.green.bold(`SUCCESS! `) + chalk.green(`Changes staged`)
    );
    ...
    if (p_1.exitCode === 128) {
      spinner.warn(
        chalk.yellow.bold(`ALERT: `) +
          chalk.yellow(
            `Current branch does not exist in remote repository yet.`
          )
      );
...
function checkChildProcess(process) {
  if (process.exitCode !== 0) {
    console.warn(
      `WARNING: Non Zero Exit code: ${process.exitCode}: ${p.command}`
    );
  }
  return process;
}
