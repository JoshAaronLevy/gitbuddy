...
module.exports = function(command, message) {
  ...
  function checkStatus() {
    ...
    .then((p) => {
      ...
      let sliceIndex = 0;
      unstagedFiles.push({name: 'All files', checked: false});
      ...
      for (let i = 0; i < fileList.length; i ++) {
        ...
        unstagedFiles.push({name: fileList[i], checked: false});
      }
      inquirer.prompt(stagingQs).then(function(answers) {
        if (answers.stageFiles[0] === 'All files') {
          answers.stageFiles = ['-A'];
          answers = 'All files';
        } else {
          return answers.stageFiles;
        }
      });
      return;
    }).catch((p) => {
...
let stagingQs = [
  {
    ...
    validate: function(answer) {
      if (answer.length > 0) {
        ...
        return "Please select the file(s) you would like to add"
      }
    }
  }
];
