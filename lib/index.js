...
let unstagedFiles = [];
let stagedFiles = [];
let updatedStagedFiles = [];
let pushFlag = false;
let currentBranch = '';
let fileList;

let thirdIndex = false;
...
const checkStatus = function() {
  ...
  .then((p) => {
    ...
    if (unstagedFiles.length === 1) {
      spinner.succeed(chalk.green.bold(`SUCCESS! `) + chalk.green(`${unstagedFiles.length} file found!`));
      return fileSelection(unstagedFiles);
    } else if (unstagedFiles.length > 1) {
      spinner.succeed(chalk.green.bold(`SUCCESS! `) + chalk.green(`${unstagedFiles.length} files found!`));
      return fileSelection(unstagedFiles);
    } else {
...
const stagingCommand = function(answer) {
  spinner = ora(`Staging ${answer.length} files...`).start();
  return execa('git', ['add', ...answer], {cwd: directory, all: true})
    .then(() => {
      if (answer.length === 1) {
        spinner.succeed(chalk.green.bold(`SUCCESS! `) + chalk.green(`${answer.length} file staged`));
      } else if (answer.length > 1) {
        spinner.succeed(chalk.green.bold(`SUCCESS! `) + chalk.green(`${answer.length} files staged`));
      }
...
const statusCheck = new MultiSelect({
  type: 'checkbox',
  name: 'stageFiles',
  message: `1/3: Which files would you like to stage for commit?`,
  choices: unstagedFiles
});

const commitMsg = new Input({
  name: 'commitInput',
  message: `2/3: What commit message would you like to use?`
});

const pushCheck = new Select({
  name: 'pushCheck',
  message: `3/3: Would you like to push to the remote repository?`,
  choices: ['Yes', 'No']
});
