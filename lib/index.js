...
module.exports = function(command, message) {
  ...
  function pushCommand() {
    const spinner = ora;
    spinner(`Pushing "${message}" to GitHub...`).start();
    const p = execa('git', ['push'], {cwd: directory})
      .then((p) => {
        ...
        spinner(`Successfully pushed "${message}" to:\n` +
        chalk.green(`  ${gitHubUrl}`)).succeed();
      }).catch((p) => {
        if (p.exitCode === 128) {
          spinner(chalk.yellow.bold(`Warning:`) + ` Current branch does not exist in GitHub yet.`).warn();
          identifyGitBranch();
        } else {
          spinner(chalk.red.bold(`Error:`) + ` Could not push to GitHub. See details below:\n` + 
          `${p.all}`).fail();
        }
      });
      return p
  ...
  function gitPushUpstream(currentBranch) {
    const spinner = ora;
    spinner(`Attempting to set ${currentBranch} as upstream and push...`).start();
    const p = execa('git', ['push', '-u', 'origin', `${currentBranch}`], {cwd: directory})
      .then((p) => {
        ...
        spinner(`Successfully set upstream and pushed to:\n` +
        chalk.green(`  ${gitHubUrl}`)).succeed();
      }).catch((p) => {
        spinner(chalk.red.bold(`Error:`) + ` Could not push to GitHub via --set-upstream. See details below:\n` + 
        `${p}`).fail();
      });
      return p
  }
