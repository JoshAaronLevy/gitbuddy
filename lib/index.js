...
const stagingCommand = function(stagedFiles) {
  ...
  return execa('git', ['add', stagedFiles], {cwd: directory, all: true})
    .then((p) => {
      ...
      return Promise.reject(new Error(`Could not stage files. ${p}`));
    });
};
  ...
  return execa('git', ['push'], {cwd: directory, all: true})
    .then((p) => {
      ...
      if (p.failed !== true) {
        ...
        return spinner.clear(`Oh no!`);
      }
    }).catch((p) => {
      // console.error(p);
      // return Promise.reject(new Error(`Could not push to remote repository. ${p}`));
      if (p.exitCode === 128) {
...
const gitPushUpstream = function(currentBranch) {
  ...
  return execa('git', ['push', '-u', 'origin', `${currentBranch}`], {cwd: directory, all: true})
    ...
    .catch((p) => {
      console.error(p);
      spinner.fail(chalk.red.bold(`Error:`) + ` Could not push to remote repository via --set-upstream. See details below:\n` + 
      `${p}`);
    });
};
