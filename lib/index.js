...
module.exports = async (command) => {
  ...
  // Commenting out block below to temporarily make push the default
...
const checkStatus = async function() {
  ...
  untrackedIndex = 0;
  try {
    // unstagedFiles.push(`Add All Files`);
    ...
    if (unstagedFiles.length === 1) {
      ...
      return spinner.warn(
        chalk.yellow.bold(`ALERT! `) + chalk.white(`No file change(s) found`)
      );
    }
  } catch (e) {
    return spinner.warn(
  ...
  if (stageFlag === true) {
    ...
    return statusCheck
      ...
      .then((answer) => {
        if (answer.length < 1) {
          return spinner.warn(
            chalk.yellow.bold(`ALERT! `) + chalk.white(`No file(s) staged`)
          );
        } else {
