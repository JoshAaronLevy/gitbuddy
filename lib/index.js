...
module.exports = (command) => {
  if (command.config && command.config === true) {
    config = true;
    return openConfig();
  } else {
...
const openConfig = function() {
  return configOptions.run().then(answer => {
    console.log(answer);
  }).catch(error => {
...
const configOptions = new Select({
  name: 'configOptions',
  message: `How would you like to handle pushing to your remote repository?`,
  choices: [
    'Prompt me each time',
    'Automatically push',
    'Never push or prompt - Commit only',
    'Cancel'
  ]
});
