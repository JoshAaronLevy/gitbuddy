...
const selectBranches = async () => {
	const spinner = createSpinner("Checking selected branches...").start();
	return branchSelect
		.run()
		.then((answer) => {
			if (answer.length < 1) {
				...
				spinner.warn({
					text: yellow(bold("ALERT! ")) + white(`Are you sure you want to delete the following branch(es)?\n${answer}`)
				});
...
const deleteSelectedBranches = async (deleteBranchList) => {
	const spinner = createSpinner("Deleting branch(es)...").start();
	try {
