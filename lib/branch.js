...
const deleteSelectedBranches = async function (deleteBranchList) {
	try {
		deleteBranchList.forEach(async (branch) => {
			spinner = ora(
			...
			try {
				...
				spinner.succeed(`Branch ${branch} deleted`);
			} catch (error) {
				spinner.fail(
					chalk.red.bold("ERROR!") +
					chalk.white(
						` Could not delete branch ${branch}. See details below:\n` +
						`${error}`
					)
				);
			}
		...
		spinner.fail(
			chalk.red.bold("ERROR!") +
			chalk.white(
				" Could not delete selected branch(es). See details below:\n" +
				`${error}`
			)
		);
	}
