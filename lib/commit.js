...
const commitCommand = async (message) => {
	...
	try {
		...
		shell.exec(`git commit -m ${commitMessage}`, { async: true, silent: true });
		spinner.success({
...
const gitPushStep = async (message) => {
	...
	try {
		const shell = await import("shelljs");
		// shell.exec("git push", { async: true, silent: true });
		return new Promise((resolve) => {
			resolve(shell.exec("git push", { async: true, silent: true }));
		}).then(() => {
