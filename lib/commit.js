...
import { red, yellow, green, white, blue, bold, underline } from "colorette";
		...
		return new Promise((resolve, reject) => {
			...
			return spinner.success({
				text: white(bold("Code changes pushed\n")) +
					white(bold("View Repo: ")) + blue(underline(`${remoteUrl}\n`)) +
					white(bold("View Branch: ")) + blue(underline(`${branchUrl}`))
			});
		}).catch((error) => {
			return spinner.error({
				text: red(bold("ERROR! ") + white(`${error}`))
