...
const commitCommand = async (message) => {
	...
	try {
		...
		const commitExec = shell.exec(`git commit -m ${commitMessage}`, { async: true, shell: "/bin/bash" }).stdout;
		// console.log("commitExec: ", commitExec);
		commitExec.on('data', (data) => {
			let splitData1 = data.split(",");
			let splitData2 = splitData1[0].split("\n");
			console.log("splitData: ", splitData2[0].split("]"));
			spinner.success({
