...
const gitPushStep = async (message) => {
	...
	try {
		...
		return spinner.succeed(
			chalk.white.bold("Code changes pushed\n") +
...
const abortCommit = async (spinner) => {
	const { ora } = await import("ora");
	const { chalk } = await import("chalk");
	spinner = ora().start();
	spinner.warn(
...
const invalidCommitMsg = async (spinner) => {
	const { ora } = await import("ora");
	const { chalk } = await import("chalk");
	spinner = ora().start();
	spinner.warn(
...
const noFilesStaged = async (spinner) => {
	const { ora } = await import("ora");
	const { chalk } = await import("chalk");
	spinner = ora().start();
	spinner.warn(
