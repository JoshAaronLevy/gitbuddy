...
module.exports = async (answers, currentBranch, message) => {
  // await logInfo(answers, currentBranch, message);
	await gitAddStep(answers, currentBranch, message);
};

const logInfo = function(answers, currentBranch, message) {
...
const gitStatusStep = function() {
	const stdout = execa('git', ['commit'], {cwd: directory})
		.then(() => {
      console.log(stdout);
      return stdout;
		}).catch((stdout) => {
