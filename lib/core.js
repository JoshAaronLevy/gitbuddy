...
const stagingCommand = async (answer) => {
	...
	try {
		...
		return await commitMessageInput();
	} catch (error) {
		return spinner.fail(
...
const commitMessageInput = async (fileCount) => {
	if (commitFlag === true) {
		message = defaultMessage;
		return await commitCommand(message);
	} else {
		return commitMsg
			.run()
			.then(async (answer) => {
				message = answer;
				if (message.length > 0) {
					return await commitCommand(message, fileCount);
				} else {
...
const commitCommand = async (message, fileCount) => {
	...
	try {
		...
		return await pushConfirm(message, fileCount);
	} catch (error) {
		return spinner.fail(
			chalk.red.bold("ERROR! ") + chalk.white(`${error}`)
		);
...
const pushConfirm = async (message, fileCount) => {
	if (pushFlag === true) {
		return await gitPushStep(message, fileCount);
	} else {
